...
void pudelka::przesun1(float x_in, float y_in)
{
	sf::Vector2f pos;
	pos.x = x_in;
	pos.y = y_in;
...
int main()
{
	...
	while (window.isOpen())
	{
		...
		if (zegar.getElapsedTime().asMilliseconds() > 30.0f) {
			...
			if (((pd7.getPos().x + 20 > p1.getPos().x) && (pd7.getPos().x > d1.getPos().x - 20)) && ((pd7.getPos().y - 20 < p1.getPos().y) && (pd7.getPos().y  < p1.getPos().y+20)))
			{

				dx = -dx;
				dy = -dy;
				p1.przesun(dx, dy);
				pd7.przesun1(1000, 1000);
			...
			window.draw(pd2.getPudelko());
			window.draw(pd3.getPudelko());
			window.draw(pd4.getPudelko());
			window.draw(pd5.getPudelko());
			window.draw(pd6.getPudelko());
			window.draw(pd7.getPudelko());
			window.display();
		}
	}
}